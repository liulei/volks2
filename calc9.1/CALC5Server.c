...
/*******************************************************************************
*/
static void calcprog_1 ()    /* server dispatch routine */
...
{
//    double ast_dopplr();
//    getCALC_arg argument = *p_request;    /* RPC caller passes this */
//    getCALC_res result  = *p_result;      /* we return this to RPC caller */
...
//    if (ifirst == 1) fflush(flog);
    ...
    result.getCALC_res_u.record.partials[0] = partials[0];
    result.getCALC_res_u.record.partials[1] = partials[1];
    result.getCALC_res_u.record.partials[2] = partials[2];
    result.getCALC_res_u.record.partials[3] = partials[3];
}

void call_calc_by_struct(PythonIn *in, PythonOut *out){

    printf("Line %d...\n", __LINE__);

    p_request   =   & argument;    /* RPC caller passes this */
    p_result    =   &result;      /* we return this to RPC caller */

    char stnnamea[8], axistypea[6];
    char stnnameb[8], axistypeb[6];
    int i;

    printf("Line %d...\n", __LINE__);

    p_request->date    =   in->date;
    p_request->time    =   in->time;
    p_request->request_id  =   150;
    p_request->ref_frame   =   0;

    for(i = 0; i < 64; ++i) p_request->kflags[i]   =   -1;
    ...
    printf("Line %d...\n", __LINE__);
    printf("length of stnname:\n");
    printf("%d\n", strlen(in->stnname));
     strcpy (stnnameb, in->stnname);
     strcpy (stnnameb, "FF");
    printf("Line %d...\n", __LINE__);
     ...
     p_request->axis_off_b = in->axis_off;

     p_request->source = &in->srcname[0];
     p_request->ra  =  in->ra;
     p_request->dec =  in->dec;
     ...
     for(i = 0; i < in->neop; ++i){
        p_request->EOP_time[i]  =   in->EOP_time[i];
        p_request->tai_utc[i]   =   in->tai_utc[i];
        p_request->ut1_utc[i]   =   in->ut1_utc[i];
        p_request->xpole[i]     =   in->xpole[i];
        p_request->ypole[i]     =   in->ypole[i];
     }
     printf("call calcprog_1 ...\n");
    calcprog_1 ();

    printf("Writing output...\n");
    out->delay  =   p_result->getCALC_res_u.record.delay[0];
    out->rate   =   p_result->getCALC_res_u.record.delay[1];
    out->u      =   p_result->getCALC_res_u.record.UV[0];
    out->v      =   p_result->getCALC_res_u.record.UV[1];
    out->w      =   p_result->getCALC_res_u.record.UV[2];
    printf("Writing partials...\n");
    out->pd_pra     =   p_result->getCALC_res_u.record.partials[0];
    out->pd_pdec    =   p_result->getCALC_res_u.record.partials[1];
}

//int main(){
void call_calc( int     date,
                double  time,   
                double  ra,
                double  dec,
                double  x,
...
/*
    long    date    =   58193;
    double  time    =   10560. / 86400.;

    double ra   =   4.675505706192551;
    double dec  =   0.168427804491194;

    double  x   =   -1281153.047069;
    double  y   =   5640864.421194;
    double  z   =   2682653.362699;
    double axis_off  =   0.004500;

    char stnname[16]    =   "KM";
    char srcname[16]    =   "J1751+0939";
*/
    getCALC_arg  req;
...
//     strcpy (srcname, "B1937+21");
     ...
     for(i = 0; i < neop; ++i){
        p_request->EOP_time[i]  =   EOP_time[i];
        p_request->tai_utc[i]   =   tai_utc[i];
        p_request->ut1_utc[i]   =   ut1_utc[i];
        p_request->xpole[i]     =   xpole[i];
        p_request->ypole[i]     =   ypole[i];
     }

/*
     for (i = 0; i < 5; i++)
     {
         p_request->EOP_time[i] = 58191.0 + (double) i; 
         p_request->tai_utc[i] = 35.0;
     }
     p_request->ut1_utc[0] = -1.846766;
     p_request->xpole[0]   = 0.014540;
     p_request->ypole[0]   = 0.369360;
     
     p_request->ut1_utc[1] = -1.847758;
     p_request->xpole[1]   = 0.014840;
     p_request->ypole[1]   = 0.371310;
     
     p_request->ut1_utc[2] = -1.848835;
     p_request->xpole[2]   = 0.014910;
     p_request->ypole[2]   = 0.373090;
     
     p_request->ut1_utc[3] = -1.849973;
     p_request->xpole[3]   = 0.015010;
     p_request->ypole[3]   = 0.374610;
     
     p_request->ut1_utc[4] = -1.851205;
     p_request->xpole[4]   = 0.015280;
     p_request->ypole[4]   = 0.376020;
*/ 
     printf("call calcprog_1 ...\n");
    calcprog_1 ();
}
...
/*++****************************************************************************
*/
int init_ast_dopplr (int argc, char *argv[])
/*
