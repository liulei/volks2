...
def main_batch(scan_no):
    ...
    d_sel   =   {}
    fname_sel   =   'No%04d_sel.npy' % (scan_no)
    if os.path.exists(fname_sel):
        d_sel   =   np.load(fname_sel, allow_pickle=True).flat[0]

    for name in names:

        if d_sel != {}:
            _name   =   name.split('/')[-1][:-4]
            if not _name in d_sel.keys():
                continue
            if d_sel[_name] != 1:
                continue
...
#        print(sp)
        ...
        if stns[0] in stns_new or stns[1] in stns_new:
            continue
        if bl_name in bls_new:
            continue
        
        if name == 'nsums/No0040_23.996416.sp.npy' and bl_name == 'MC-SR':
            print('skip %s, bl %s ...' % (name, bl_name))
            continue
